...
@measure_energy
def unrolled_loop(sorted_collection, item, left, right):
    while left <= right:
        ...
        # Second unrolled iteration (if left <= right)
        if left <= right:
            midpoint = left + (right - left) // 2
            current_item = sorted_collection[midpoint]
...
unrolled_loop(sorted_collection, item, 0, len(sorted_collection) - 1)
