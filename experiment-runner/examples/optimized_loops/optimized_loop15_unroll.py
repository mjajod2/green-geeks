...
@measure_energy
def unrolled_loop(current_state, max_x, min_x, max_y, min_y, find_max, max_iter):
    iterations = 0
    visited = set()
    scores = []
    ...
    while not solution_found and iterations < max_iter:
        ...
        for i in range(0, len(neighbors), 2):
            ...
            if neighbor in visited:
                continue
            if not(min_x <= neighbor.x <= max_x and min_y <= neighbor.y <= max_y):
                continue
            ...
            # Process second neighbor in unrolled iteration
...
unrolled_loop(current_state, 100, -100, 100, -100, True, 100)
