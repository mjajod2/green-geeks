...
@measure_energy
def unrolled_loop(mock_function_call, median_of_five, mock_variable_assignment, arr):
    i = 0
    medians = []
    len_arr = len(arr)
    ...
    while i < len_arr:
        ...
        # Second unrolled iteration (to process another 5 elements in one loop)
        if i >= len_arr:
...
unrolled_loop(mock_function_call, median_of_five, mock_variable_assignment, arr)
