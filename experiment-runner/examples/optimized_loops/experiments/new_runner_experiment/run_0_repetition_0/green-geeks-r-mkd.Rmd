...
qqnorm(positive_energy_data$duration, main="Q-Q Plot for duration")
...
ggplot(positive_energy_data, aes(x = example_factor1, y = package_0)) +
  ...
  geom_boxplot(width = 0.1) +  # Remove outliers from boxplot
  scale_y_continuous(limits = c(0, 2e8)) +  # Set y-axis limit to zoom in
  labs(title = "Violin Plot of Energy Consumption by Optimization Technique",
...
kruskal_test_energy <- kruskal.test(package_0 ~ example_factor1, data = positive_energy_data)
kruskal_test_energy

kruskal_test_duration <- kruskal.test(duration ~ example_factor1, data = positive_energy_data)
kruskal_test_duration

kruskal_test_cpu <- kruskal.test(cpu_power ~ example_factor1, data = positive_energy_data)
kruskal_test_cpu
```
...
dunn_test_result <- dunn.test(positive_energy_data$duration, positive_energy_data$example_factor1, method = "bonferroni")
...
cor.test(positive_energy_data$cpu_power, positive_energy_data$package_0, method = "spearman")
```
