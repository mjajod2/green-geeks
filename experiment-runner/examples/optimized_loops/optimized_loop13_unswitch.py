...
@measure_energy
def unswitched_loop(current_state, max_x, min_x, max_y, min_y, find_max, current_temp, rate_of_decrease, threshold_temp):
    search_end = False
    best_state = None
    iterations = 0
    scores = []
    ...
    while not search_end:
        ...
        # Move boundary checks outside the loop
        while next_state is None and neighbors:
            index = random.randint(0, len(neighbors) - 1)
            picked_neighbor = neighbors.pop(index)
            change = picked_neighbor.score() - current_score

            valid_x_range = min_x <= picked_neighbor.x <= max_x
            valid_y_range = min_y <= picked_neighbor.y <= max_y

            if valid_x_range and valid_y_range:
                if not find_max:
                    change *= -1
                if change > 0:
...
unswitched_loop(current_state, 100, -100, 100, -100, True, 1000, 0.1, 1)
